...
def add_overlays(frame, faces, frame_rate,today_date):
    ...
    if faces is not None:
        ...
        for face in faces:
            ...
            print("latest print date ",date_time_str)
            ...
            if face.name != 'unknown':
                ...
                Insert_data = "INSERT INTO " + DB_table_name + " (NAME,CHECKIN) VALUES (%s, %s)"
                VALUES = (str(face.name ), str(date_time_str))

                try:
                    cursor.execute(Insert_data, VALUES)
                    db.commit()

                    print("i written to db")
...
#video_capture = VideoStream('rtsp://ashlesha:green120@192.168.0.9:554/Streaming/Channels/101').start()
